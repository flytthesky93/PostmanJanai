...
Nếu release này có thay đổi **Phase 9 (Scripting)** → thêm tick nhanh:
...
- [ ] Tab **Pre-request** + **Tests** trong RequestPanel mở được; nhập script `pm.environment.set('token', pm.response.json().token)` ở post-response → Send → biến `token` cập nhật trong env active.
