...
> **Đóng quy trình plan v1 — 2026-04-30:** Phase **0–9** hoàn thành. Không còn **backlog tích lũy** trong tài liệu; mở rộng sau này ghi vào **Phase 10+** (thêm một dòng trong bảng bên dưới và mô tả scope khi bắt đầu). `release-checklist.md` / `manual-test-plan.md` vẫn dùng cho **gate từng bản release**.
...
**Đóng quy trình v1:** Các ý tưởng không thuộc Phase 5 (Playwright E2E, export native DB migrate giữ data, polish export URL `{{var}}`, ký/notarize…) **không còn theo dõi như backlog** — chuyển sang **phase mới** khi có kế hoạch triển khai.
...
**Đóng backlog tài liệu — mở rộng chỉ qua phase mới:** OAuth2, mTLS, PAC proxy, encrypt-at-rest cho DB chưa nằm trong phạm vi Phase 6 đã ship; không còn danh sách backlog dự án cho các mục này.
...
**Đóng backlog tài liệu:** Theme/light mode toggle, request/response diff viewer — không theo dõi như backlog; nếu làm sẽ ghi trong **phase mới**.
...
**Đóng backlog tài liệu:** Data-driven Runner (CSV/JSON), parallel, retry — chỉ được mô tả khi có **phase Runner mới**.
...
**Đóng backlog tài liệu — scripting nâng cao chỉ trong phase mới:** full pm/Chai, cookie jar, visualizer, async/Promise, debugger, script cấp collection, ES modules/CommonJS không còn được theo dõi như **Phase 9.1 backlog** của dự án.
...
## Tiếp theo sau v1 (đóng backlog tài liệu — 2026-04-30)

**Không còn backlog tích lũy trong plan.** Phase 0–9 đã đóng. Việc triển khai mới (sau này) ghi trong **Phase 10+** (hoặc tên phase mới) trong bảng Phase completion log khi bạn bắt đầu kế hoạch.

**Lưu ý vận hành:** Bản release internal vẫn dùng `release-checklist.md` + smoke theo từng bản build; đó là **gate chất lượng**, không phải backlog năng lực.

### Lịch sử ưu tiên (đã hoàn thành — giữ để tham chiếu)

1. ~~Workspace UI CRUD~~ **Done (Phase 0).**
2. ~~RequestExecutor + HTTPHandler~~ **Done.**
3. ~~RequestPanel ↔ backend~~ **Done.**
4. ~~History persist~~ **Done.**
5. ~~Folder + saved request~~ **Done (Phase 2).**
6. ~~Environments + auth~~ **Done (Phase 3).**

### Lịch sử phase 6–9 (đã hoàn thành)

- ~~Phase 6~~ **Done** — proxy, CA, secret env, Settings.
- ~~Phase 7~~ **Done** — Dashboard, palette, shortcuts, code splitting.
- ~~Phase 8.0 + 8.1~~ **Done** — Runner, capture/assertion, raw replay, iterations/delay/timeout.
- ~~Phase 9~~ **Done** — Scripting, DB v9.

**Nguyên tắc đã áp dụng (tham chiếu cho phase sau):** mở rộng manual test + checklist theo phase; bump DB kèm test `data_migrate_test.go`; thay đổi HTTP/usecase lớn kèm smoke E2E khi cần.

**Bảng hạng mục (chỉ lịch sử — không còn hàng backlog mở)**
...
| ~~**7**~~ | ~~Migrate DB v2 → v3 giữ dữ liệu~~ | **Không theo dõi trong plan** — nếu làm, nhập vào phase mới. |
| ~~**8**~~ | ~~Export project JSON native~~ | **Không theo dõi trong plan** — nếu làm, nhập vào phase mới. |
...
| ~~**13**~~ | ~~**Scripting** — goja + `pmj`/`pm` subset~~ | **Done (Phase 9 — 2026-04-30, DB v8 → v9).** |
