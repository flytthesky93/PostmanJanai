...
// The TrustedCAFunc type is an adapter to allow the use of ordinary
// function as TrustedCA mutator.
type TrustedCAFunc func(context.Context, *ent.TrustedCAMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TrustedCAFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TrustedCAMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TrustedCAMutation", m)
}
