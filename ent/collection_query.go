...
func (_q *CollectionQuery) QueryWorkspace() *WorkspaceQuery {
...
func (_q *CollectionQuery) QueryRequests() *RequestQuery {
...
func (_q *CollectionQuery) WithWorkspace(opts ...func(*WorkspaceQuery)) *CollectionQuery {
	query := (&WorkspaceClient{config: _q.config}).Query()
...
func (_q *CollectionQuery) WithRequests(opts ...func(*RequestQuery)) *CollectionQuery {
	query := (&RequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(collection.FieldWorkspaceID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Collection.Query().
//		Select(collection.FieldWorkspaceID).
//		Scan(ctx, &v)
...
func (_q *CollectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Collection, error) {
	...
	if query := _q.withWorkspace; query != nil {
		if err := _q.loadWorkspace(ctx, query, nodes, nil,
			func(n *Collection, e *Workspace) { n.Edges.Workspace = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *CollectionQuery) loadWorkspace(ctx context.Context, query *WorkspaceQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *Workspace)) error {
...
func (_q *CollectionQuery) loadRequests(ctx context.Context, query *RequestQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *Request)) error {
...
func (_q *CollectionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withWorkspace != nil {
			_spec.Node.AddColumnOnce(collection.FieldWorkspaceID)
		}
	}
