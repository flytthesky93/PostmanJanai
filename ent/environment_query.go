...
type EnvironmentQuery struct {
	...
	withEnvironmentVariables *EnvironmentVariableQuery
...
// QueryEnvironmentVariables chains the current query on the "environment_variables" edge.
func (_q *EnvironmentQuery) QueryEnvironmentVariables() *EnvironmentVariableQuery {
...
func (_q *EnvironmentQuery) Clone() *EnvironmentQuery {
	...
	return &EnvironmentQuery{
		...
		withEnvironmentVariables: _q.withEnvironmentVariables.Clone(),
...
// WithEnvironmentVariables tells the query-builder to eager-load the nodes that are connected to
// the "environment_variables" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *EnvironmentQuery) WithEnvironmentVariables(opts ...func(*EnvironmentVariableQuery)) *EnvironmentQuery {
	query := (&EnvironmentVariableClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withEnvironmentVariables = query
	return _q
}
...
func (_q *EnvironmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Environment, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [1]bool{
			_q.withEnvironmentVariables != nil,
		}
	)
	...
	if query := _q.withEnvironmentVariables; query != nil {
		if err := _q.loadEnvironmentVariables(ctx, query, nodes,
			func(n *Environment) { n.Edges.EnvironmentVariables = []*EnvironmentVariable{} },
			func(n *Environment, e *EnvironmentVariable) {
				n.Edges.EnvironmentVariables = append(n.Edges.EnvironmentVariables, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *EnvironmentQuery) loadEnvironmentVariables(ctx context.Context, query *EnvironmentVariableQuery, nodes []*Environment, init func(*Environment), assign func(*Environment, *EnvironmentVariable)) error {
	...
	query.Where(predicate.EnvironmentVariable(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(environment.EnvironmentVariablesColumn), fks...))
	}))
